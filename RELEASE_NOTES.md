...
Bundle 6.3.0 advances Long Document from 1.0.0 to 1.1.3, Skill Evaluator from 3.2.0 to 3.3.4, Software Quality Workflows from 9.0.0 to 9.0.4, and Writing Plans from 8.1.0 to 8.2.6. Schema epoch 5, the mixed activation matrix, and the explicit-only evaluator boundary remain unchanged. It adds a bounded model-evolution controller, six inert Codex Host probes, exact four-Skill non-ready sentinel suites, deterministic suite-quality evidence, external holdout contracts, and fresh-process qualification verification.

Long Document 1.1.3 preserves explicitly requested recovery or audit markers in the reader-facing deliverable and makes compact output-only recovery blocks structurally exact. Skill Evaluator 3.3.4 preserves trusted fixture-relative identities, redacts local roots, gives equal-length evidence paths a deterministic total order, and retains the literal `$SKILL_EVALUATOR_DIR` owner in delivered commands instead of leaking installation paths. Software Quality Workflows 9.0.4 treats an explicitly settled exact disposition as binding, deletes tests owned solely by retired behavior, and uses one bounded controller ledger for durable resume boundaries. Writing Plans 8.2.6 keeps bounded named-file plans native, requires explicit Program edits to preserve observed transformations, spells out every milestone dependency, uses identifier-aware residual checks for renames, derives the test working directory and module path from observed source, and does not inherit Program proof defaults. The finite sentinel benefit gate combines a non-inferiority interval with zero candidate failures and positive baseline headroom; headroom cases keep target contracts out of the no-Skill task fixtures. SQW planning artifacts are judged directly while role-only ownership and syntax-only behavior checks remain failures.

The controller reuses the existing evaluator compiler, runner, analyzer, comparison, plugin builder, static diagnostic, and release authorization. It owns no grader, reviewer, optimizer, durable worker, cache, retry loop, installation, or release authority. Unknown Host capabilities and absent calibration or holdout evidence remain explicit limits or blockers; generated source-complete evidence never becomes a model-support claim. Calibration abstentions use genuinely truncated or conflicting observations, while positive process examples name the concrete mechanism evidence required by their grader contract. Long Document grading keeps the intermediate compaction anchor separate from the final resumed report so the compact-recovery packet rule cannot leak across case identities.
