...
Long Document 1.1 preserves explicitly requested recovery or audit markers in the reader-facing deliverable. Writing Plans 8.2.3 keeps bounded named-file plans native, requires complete runnable bodies for exact tests, derives the test working directory and module path from observed source, and does not inherit Program proof defaults. The finite sentinel benefit gate combines a non-inferiority interval with zero candidate failures and positive baseline headroom; SQW planning artifacts are judged directly while role-only ownership and syntax-only behavior checks remain failures.
