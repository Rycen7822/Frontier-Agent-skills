...
Bundle 6.3.0 advances Long Document from 1.0.0 to 1.1.0, Skill Evaluator from 3.2.0 to 3.3.3, Software Quality Workflows from 9.0.0 to 9.0.2, and Writing Plans from 8.1.0 to 8.2.4. Schema epoch 5, the mixed activation matrix, and the explicit-only evaluator boundary remain unchanged. It adds a bounded model-evolution controller, six inert Codex Host probes, exact four-Skill non-ready sentinel suites, deterministic suite-quality evidence, external holdout contracts, and fresh-process qualification verification.

Long Document 1.1 preserves explicitly requested recovery or audit markers in the reader-facing deliverable. Skill Evaluator 3.3.3 preserves trusted fixture-relative directory identities while redacting local roots and gives equal-length evidence paths a deterministic total order. Software Quality Workflows 9.0.2 treats an explicitly settled exact disposition as binding, deletes tests owned solely by retired behavior, and does not infer retargeting from the existence of a replacement API. Writing Plans 8.2.4 keeps bounded named-file plans native, requires complete runnable bodies for exact tests, uses identifier-aware residual checks for renames, derives the test working directory and module path from observed source, and does not inherit Program proof defaults. The finite sentinel benefit gate combines a non-inferiority interval with zero candidate failures and positive baseline headroom; headroom cases keep target contracts out of the no-Skill task fixtures. SQW planning artifacts are judged directly while role-only ownership and syntax-only behavior checks remain failures.
