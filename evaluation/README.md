...
`offline-route-replay.json` is a deterministic replay of 25 semantically aligned v4/v3 and vNext curated route cases. It is generated by `scripts/evaluate_offline_route_replay.py` from the frozen baseline revision and current exact bundle. It currently proves 100% curated exact-primary routing, one-card limits, zero mandatory truncation in the replay, an M0 median/p95 of 3,404/4,044 active reference bytes, and an 88.2% median reference-byte reduction. It explicitly records that hidden routing, natural model behavior, outcome quality, and publication readiness remain untested.
