...
def isolated_child_argv(
    *,
    ...
    masked_roots = (repository_root, global_codex_home, Path("/tmp"))
    if any(codex.is_relative_to(root) for root in masked_roots):
    ...
    return [
        ...
        *executable_mount,
        ...
        "--tmpfs", str(repository_root),
        "--tmpfs", str(global_codex_home),
        "--dev", "/dev",
        "--proc", "/proc",
        "--unshare-pid",
        "--as-pid-1",
