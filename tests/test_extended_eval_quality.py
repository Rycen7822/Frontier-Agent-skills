...
class TestExtendedEvalQuality(SkillEvaluatorTestCase):  # noqa: F405
    ...
    def test_reviewer_pair_supports_disjoint_campaign_partitions(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            paths = self._materialize_high_risk_reviewer_pair(Path(tmp))
            labels = [
                json.loads(line)
            ...
            opaque_by_example = {
                item['example_id']: item['opaque_example_id']
            ...
            reviewer_rows = [
                row for row in ratings
                if row['reviewer']['role']
                == 'context_clean_subagent_reviewer'
            ]
            midpoint = len(labels) // 2
            partitions = (labels[:midpoint], labels[midpoint:])

            def validate_partition(partition: list[dict]) -> dict:
                opaque_ids = {
                    opaque_by_example[row['example_id']]
                    for row in partition
                }
                return contract.validate_reviewer_pair(
                    paths['reviewer_pair'],
                    output_root=paths['calibration'].parent,
                    reviewer_rows=[
                        row for row in reviewer_rows
                        if row['example_id'] in opaque_ids
                    ],
                    label_rows=partition,
                    judge_reviewer_ids={
                    ...
                    judge_grader_id=judge_rows[0]['grader_id'],
                )

            with self.assertRaisesRegex(
                contract.ReviewerPairError,
                'declared mapping partition',
            ):
                validate_partition(partitions[0])
            mapping['partitions'] = [
                {
                    'partition_id': f'campaign-partition-{ordinal}',
                    'opaque_example_ids': [
                        opaque_by_example[row['example_id']]
                        for row in partition
                    ],
            ...
            for partition in partitions:
                ...
                observed_examples.update(
                    row['example_id'] for row in result['mapped_rows']
                )
            self.assertEqual(
    ...
    def test_reviewer_pair_rejects_partition_gap_and_overlap(self) -> None:
        for mutation in ('gap', 'overlap'):
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as tmp:
                paths = self._materialize_high_risk_reviewer_pair(Path(tmp))
                mapping = json.loads(
                    paths['reviewer_mapping'].read_text(encoding='utf-8'),
                )
                opaque_ids = mapping['partitions'][0]['opaque_example_ids']
                mapping['partitions'][0]['opaque_example_ids'] = (
                    opaque_ids[:-1]
                    if mutation == 'gap'
                    else [opaque_ids[0], *opaque_ids]
                )
                self._close_self_hash(mapping, 'mapping_hash')
                self._write_json(paths['reviewer_mapping'], mapping)
                self._rebind_pair(paths)
