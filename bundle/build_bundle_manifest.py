#!/usr/bin/env python3
"""Build or verify the exact four-skill Frontier 5.0 bundle manifest."""
...
def _skill_record(skill_id: str, version: str) -> dict[str, Any]:
    ...
    if activation is not EXPECTED_ACTIVATION[skill_id]:
        raise ValueError(f"skill activation differs from the fixed 5.0 matrix: {skill_id}")
...
def build_manifest() -> dict[str, Any]:
    ...
    if source.get("bundle_schema_version") != "3.0" or source.get("bundle_version") != "5.0.0":
        raise ValueError("source bundle must bind schema 3.0 and release 5.0.0")
