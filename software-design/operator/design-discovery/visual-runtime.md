...
Resolve the active `software-design` skill root from the host's loaded-skill metadata or installed path, then launch its script through the host's tracked long-running command/process capability. Require a process or session handle and wait for the `server-started` startup record:
