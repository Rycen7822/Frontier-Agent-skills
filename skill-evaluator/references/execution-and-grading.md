...
This file owns run isolation, run index v1, receipt v1, artifact/provenance verification, routing/usage/context capture, grader transport/semantics, and apparatus failure classification.
...
```json
{
  ...
  "artifacts": [{"path": "trace.jsonl", "sha256": "sha256:<64 hex>", "encoding": "utf-8"}],
  "routing": {},
  "usage": {},
  "context_usage": {},
  "grader_outputs": []
...
Provenance binds spec, case, environment, fixture, selected grader set, package, catalog, and treatment identities. Candidate package inventory, fixture manifest/files, grader declarations, verifier bytes, model prompt, and model schema are recomputed locally. Catalog/treatment/controller claims remain external attestations.

## Routing and usage

Routing records unique retrieved IDs, selected ID, body load, unique resources, incorporation, application, and evidence line locators. Eligibility is derived from case and variant mode, never supplied as a denominator switch.

Usage records non-negative input/output tokens, latency, tool calls, retries, and evidence. These are end-to-end operational costs. They do not prove target-Skill context attribution.
...
Model rubric output appears once as `{grader_id,output}` and cannot carry an invocation. Deterministic output cannot be supplied inline. Both use the same semantic validator.
