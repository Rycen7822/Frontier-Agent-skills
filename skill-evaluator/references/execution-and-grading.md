...
This file owns deterministic plan compilation, bounded runner behavior, host protocol, run index v3, receipt v5, artifact custody verification, grader execution/verification boundaries, resume, and apparatus failure classification.
...
`compile_eval_plan.py` is a pure projection. It starts no host, grader, verifier, or subject; reads no prior runtime result; and uses no wall clock, PID, process digest, or temporary path. It validates all bound inputs, rejects non-ready contracts and failed preparation gates, expands the exact scenario × treatment × repeat matrix, counterbalances deterministically, derives capability-based dispositions, and writes one schema-valid plan.

Every entry is exactly `execute`, `unsupported`, or `not_evaluable`. Unknown required capability evidence takes precedence over unsupported evidence. Non-execute entries preserve their feasibility facts but receive no workspace, request, attempt, receipt, or index row. Plan and entry IDs are readable structural locators. The runner binds the complete plan bytes once in the index header; IDs do not substitute for that custody check.
...
The first JSONL record is the only index header and contains `record_type=index_header`, plan ID, and one plan-bytes digest. Each later attempt record contains readable entry/run/attempt IDs, artifact directory, and one receipt `{path,digest}` binding. Attempt records contain no plan digest, pass, score, routing, usage, grader, feasibility, or provenance claim. The runner requires `--index` to resolve to the plan-declared path under `artifacts.root`.

## Default evidence traversal

Raw scored receipts and their artifacts stay immutable for the frozen retention period. The default reading path is analyzer summary → failure index → spec-limited representative receipts. Open raw artifacts through one named receipt for a named failure, grader disagreement, or integrity audit. This path keeps the working context bounded while preserving exact drill-down evidence.

## Receipt v5

One receipt owns the complete attempt evidence: readable run/provenance facts, artifacts, host protocol, routing, principals, handoffs, actions, observations, state, faults, usage, context usage, grader outputs, and cleanup. It binds plan, entry, scenario, host, package, catalog, treatment, fixture, grader, and conditionally calibration/quality semantics; each external owner retains its byte custody binding.

The analyzer requires exact shapes. It normalizes POSIX-relative paths, rejects absolute/backslash/parent traversal, proves lexical and symlink-resolved containment, verifies each independently consumed raw artifact digest, and rejects duplicate normalized or resolved artifacts. Every evidence locator and invocation references the exact artifact allowlist.

Index v3 owns the receipt byte binding. Receipt v5 owns attempt semantics and independently consumed raw-artifact bindings. `completion_origin=resume_seal` identifies the schema-declared recovered-attempt form.
...
`grader_semantics.py` owns the exact `{view,check:{check_id,pass_condition}}` payload used by formal requests, calibration v3, and optional reviewer packets. Calibration gold owns one payload digest; Host-normalized requests and reviewer outputs use ordered semantic IDs and judgment values without returning that digest. Absolute workspace locators and caller-defined forbidden view fields fail at the originating boundary.
...
When required, the only manual authority input is `--manual-review-receipt <relative-path>` under `spec.artifacts.root`. It contains exact reviewer role, one external artifact binding per required evidence type, `approve|hold|reject`, and a non-empty signature attestation. The analyzer checks containment, bound bytes, role, evidence-type closure, and decision; it does not perform cryptographic signature verification.
