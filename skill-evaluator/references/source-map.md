...
Local adaptation: spec v5 and scenario v1 bind requirements, execution identity, preparation, receipts, artifacts, and grader evidence instead of trusting host self-report. Codex event names remain examples rather than portable requirements.
...
|---|---|---|
| `schema_version=5`, `execution.ready`, `analysis.estimands[]`, `hard_gates[]` | M-01 | OpenAI measurable success; Pattern plan-before-action |
...
| scenario v1 `requirements[]`, exact graders, receipt v4 grader outputs | M-03 | OpenAI deterministic and qualitative grading |
| `audit_skill_package.py::audit`, reachable schemas/support, scan completeness | M-04, M-05 | Anthropic package anatomy/security |
| `treatment_id`, execution identity, `compile_plan`, plan/entry hashes | M-06 | Ding et al. relevance/execution/reuse; Pattern frozen scope |
...
| Stateful work needs obligations, transitions, terminal, and cleanup facts | M-05, R-14 | scenario turns/state; receipt v4 state | `tests/test_extended_eval_execution.py::test_runner_closes_state_principal_and_handoff_contracts` |
| Recovery verifies durable state rather than narrative | R-12, E-04 | attempt marker/index/receipt; runner `--resume` | `tests/test_extended_eval_execution.py::test_resume_seals_marker_only_attempt_without_inventing_outcome` |
...
| Handoffs lose provenance unless payload, authority, raw result, and transforms are typed | R-17, E-10 | host `$defs`; receipt v4 handoffs | `tests/test_extended_eval_execution.py::test_async_delivery_preserves_forward_causal_ancestry` |
...
| Default views must be compact while drill-down stays exact; external authority stays separate | A-01–A-17 | summary v4, failure index v1, output manifest, manual authority | `tests/test_extended_reporting.py::test_v5_analyzer_writes_compact_bound_views` |
...
- spec v5, scenario v1 `requirements[]`, and applicability registry;
- execution plan v1, run-index row v2, and receipt v4 as the only runtime evidence path;
