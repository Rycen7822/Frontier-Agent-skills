...
| Pattern evidence synthesis | Plan requirement | Current field/function owner | Acceptance evidence |
|---|---|---|---|
| Workflows route on facts instead of one universal sequence | C-06, M-01 conditional modules | spec `applicability`; compiler module projection | compiler validation and generated plan |
| Plans freeze scope, controls, authority, and stop conditions before action | P-01–P-10 deterministic plan | `compile_eval_plan.py::compile_plan` | public compile-run-analyze lifecycle |
| Stateful work needs obligations, transitions, terminal, and cleanup facts | M-05, R-14 | scenario turns/state; receipt v5 state | scenario/receipt schema validation |
| Recovery verifies durable state rather than narrative | R-12, E-04 | attempt custody/status/budget; marker/index/receipt; runner `--resume` | runner status and campaign preflight |
| Crowded catalogs expose overlap, no-match, order, and context cost | M-02 | scenario `routing_contract`; runner routing validator | contract validation and formal campaign |
| Composition requires exact participants, order, and handoff evidence | M-03, M-04 | routing composition; typed handoffs | contract validation and formal campaign |
| Topology claims need decomposability and equal-budget single-principal control | C-16, M-18 | scenario coordination; plan/receipt principals | contract validation and formal campaign |
| Handoffs lose provenance unless payload, authority, raw result, and transforms are typed | R-17, E-10 | host `$defs`; receipt v5 handoffs | host/receipt schema validation |
| Tool availability, authorization, delivery, rendering, and effects are distinct stages | R-18, R-19, M-21, M-23 | receipt actions; runner action validator; `action_summary` | host/receipt schema validation |
| Causal attribution requires fixed execution identity and one intervention axis | C-15 | spec treatments; plan identity | compiler validation and generated plan |
| Suite validity and blinded judge calibration precede scoring | C-10–C-12, R-08 | validator calibration/suite-quality producers | contract validation and formal campaign |
| Shared lineage/context is dependent; critique must be taken up and repaired | M-19, M-20, E-09 | independence/critique summaries | analyzer projection and formal campaign |
| Missing/tampered evidence and unsupported capability are not candidate failures | C-18, M-11, M-22 | disposition, feasibility, evidence axes | analyzer projection and formal campaign |
