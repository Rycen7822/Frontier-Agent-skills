...
For a proposed edit, name the missing or conflicting rule, owning file, visible trigger and action boundary. Explain how the smallest replacement addresses the observed problem and when it stops applying or needs new evidence. Keep one-off checks with the task instead of turning them into shared instructions. Prioritize recurring or severe gaps; do not rerun tasks to manufacture recurrence. If a clear rule was ignored, distinguish guidance unavailable at the decision from model variance or a failure in code, infrastructure or grading. Recommend moving existing guidance or using its owning check only when the episode supports that cause; do not repeat the rule. Drop issues already fixed in the current state. A successful session can contain fixable waste, but no verified gap means no Skill edit.
