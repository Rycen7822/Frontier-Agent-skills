...
def _mutate(state: dict, name: str) -> None:
    if name == "schema_unknown_field":
        ...
        state["policy_claims"].append({"policy_id": "sqw.verify.completion-evidence", "bundle_version": "frontier-engineering/7.0.0+6.0.0", "policy_hash": "sha256:" + "5" * 64})
...
class PlanStateTests(unittest.TestCase):
    ...
    def test_v3_policy_invariant_effect_and_bundle_identity_are_schema_owned(self) -> None:
        ...
        self.assertEqual("frontier-engineering/7.0.0+6.0.0", state["bundle_id"])
