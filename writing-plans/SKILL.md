...
A bounded single-session request naming files and checks must end as a native ordered plan even when Git identity, dirty/protected paths, or exact source identity are visible; omit those facts unless the request names them, and do not apply the Handoff/Program contract below. Inspect each available bound file once. State observed symbols and behavior, exact edits, checks, expected results, and failure exits. Exact means observed old→new symbols/expressions, argument-parser calls, literal doc text, complete runnable test bodies when exact tests are requested, and runnable assertions for exact checks; generic "add parsing", prose-only tests, or "follow existing conventions" fails. For a symbol rename, make the residual check identifier-aware: test old-name absence and new-name presence against the same collected identifier set; never use a raw substring test when the new identifier contains the old one, or compare identifier text with a tokenizer token-type constant. Derive one starting cwd and module path from observed imports/layout; never `cd` into the stated cwd again. Include `PYTHONPATH` when needed: before return, resolve each test import from that cwd; if its package root is nested, set that root for the runnable command. Use only bound checks executable in the workspace. A native proof never adds whole-file snapshots, Git status/diff scope, protected-boundary, residue, whitespace, rollback, cleanup, attestation, combined-only proof, or repository-wide checks unless the request or bound source names that exact check. An explicit non-Git identity forbids Git status, diff, or rollback. Do not add contract rows or unrequested owners.
