...
## Required inputs
- Source/scope identities, requested plan/profile/audience, canonical final-output location, authority, software decision questions, requirements, unresolved decisions, and bundle-resolved owner availability.

## Procedure
1. Route corpus management, source ledger, segmented drafting, recovery packets, deterministic assembly, and whole-draft review to the bundle's long-document segmented-writing owner when available and justified.
...
5. If the bundle owner is unavailable, stop with a typed handoff blocker unless the corpus is demonstrably bounded for local segmented handling.

## Output contract
- One `long-document-handoff` containing the bundle owner route, `scratch_retention`, final locator/hash, source/scope identities, requirements status, unresolved software decisions, and `blocker|null`. It is a delivery boundary, not an owner anchor.
...
## Stop
Stop without claiming corpus coverage when the bundle owner cannot run and a bounded fallback is not justified.
