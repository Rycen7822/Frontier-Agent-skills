...
Card-cycle runtime errors are `E_COMMAND_SCHEMA`, `E_COMMAND_BUDGET`, `E_CONTRACT_INVALID`, `E_RECEIPT_INVALID`, `E_ROOT_ROLE`, `E_UNSUPPORTED_VARIANT`, `E_ORPHAN_CONFLICT`, `E_STATE_STALE`, `E_STATE_ADVANCED`, `E_STATE_BUDGET`, `E_SOURCE_UNAVAILABLE`, `E_SOURCE_DRIFT`, `E_SOURCE_REVISION_CHANGED`, `E_POST_PUBLISH_UNVERIFIED`, `E_PROJECTION_BUDGET`, `E_CONTEXT_NOT_CURRENT`, and `E_HANDOFF_INVALID`. `E_POST_PUBLISH_UNVERIFIED` preserves the immutable final so the same operation can verify and replay it exactly. Errors never authorize fallback writes, profile switching, truncation, queue edits, or cleanup of foreign bytes.
