#!/usr/bin/env python3
"""Validate the non-loading SQW normative policy ownership graph."""
...
from _workflow_reference_cards import BUNDLE_ID, SKILL_ID, TARGET_SKILL_VERSION, load_json


ROOT = Path(__file__).resolve().parents[1]
REGISTRY = ROOT / "registries" / "policy-owners.json"
MANIFEST = ROOT / "registries" / "reference-cards.manifest.json"
POLICY_ID = re.compile(r"^sqw(?:\.[a-z0-9][a-z0-9-]*)+$")


def validate(registry: Any, manifest: Any) -> list[dict[str, str]]:
    issues: list[dict[str, str]] = []
    expected_keys = {"bundle_id", "policies", "schema_version", "skill_id", "skill_version"}
    ...
    if not isinstance(policies, list) or not policies:
        return issues + [{"code": "policy.list", "path": "/policies", "message": "policies must be a non-empty array"}]
    by_id: dict[str, dict[str, Any]] = {}
    for index, policy in enumerate(policies):
        path = f"/policies/{index}"
        if not isinstance(policy, dict) or set(policy) != {"depends_on", "owner_id", "owner_type", "policy_id"}:
            issues.append({"code": "policy.shape", "path": path, "message": "policy has unexpected keys"})
            continue
        policy_id = policy.get("policy_id")
        if not isinstance(policy_id, str) or not POLICY_ID.fullmatch(policy_id) or policy_id in by_id:
            issues.append({"code": "policy.id", "path": path + "/policy_id", "message": "policy ID is invalid or duplicate"})
            continue
        by_id[policy_id] = policy
        owner_type = policy.get("owner_type")
        owner_id = policy.get("owner_id")
        if owner_type == "card":
            if owner_id not in cards:
                issues.append({"code": "policy.owner-missing", "path": path + "/owner_id", "message": "card owner is absent from manifest"})
            ...
            if not isinstance(owner_id, str) or owner_id.startswith("/") or ".." in Path(owner_id).parts or not (ROOT / owner_id).is_file():
                issues.append({"code": "policy.owner-missing", "path": path + "/owner_id", "message": "machine owner is unsafe or missing"})
        else:
            issues.append({"code": "policy.owner-type", "path": path + "/owner_type", "message": "owner_type must be card or machine"})
        ...
        if not isinstance(depends, list) or any(not isinstance(item, str) for item in depends) or len(depends) != len(set(depends)):
            issues.append({"code": "policy.depends", "path": path + "/depends_on", "message": "depends_on must be a unique string array"})
    visiting: set[str] = set()
    visited: set[str] = set()

    def visit(policy_id: str) -> None:
        if policy_id in visiting:
            issues.append({"code": "policy.cycle", "path": policy_id, "message": "policy dependency graph contains a cycle"})
            return
