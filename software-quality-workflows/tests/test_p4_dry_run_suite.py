...
SCENARIOS = ROOT / "tests" / "fixtures" / "closure" / "p4-dry-run-scenarios.json"
TRAJECTORIES = ROOT / "tests" / "fixtures" / "closure" / "controller-trajectories.json"


class P4DryRunRepositorySuite(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            for index, scenario in enumerate(scenarios, 1):
                with self.subTest(scenario=scenario["id"]):
                    ...
                    self.assertFalse(_git(repo, "remote"), "dry-run repo must have no remote")
