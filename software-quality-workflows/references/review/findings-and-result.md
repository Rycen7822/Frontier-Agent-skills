...
## Procedure
1. Emit one immutable revision-bound result with reviewed base/head revisions, explicit reviewed paths, separate local code verdict, verification status, traceability status/evidence, exact full/sampled/not-reviewed coverage, blockers, non-finding risks, summary, and positive notes. The terminal revision/path binding carries local identity; a publication owner may add one outer digest when the bytes cross that boundary.
