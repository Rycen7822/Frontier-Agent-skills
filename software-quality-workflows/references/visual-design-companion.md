...
- Use a task-owned `/tmp` session by default. Pass `--project-dir` only when persistent mockups are wanted; this creates `<project>/.hermes-design-discovery/`, whose tracked/ignored status must be checked before closeout.
...
Resolve the active `software-quality-workflows` `skill_dir` from `skill_view`, then launch its script as a tracked long-lived process with `terminal(background=true, watch_patterns=["server-started"])`:

```text
terminal(
  command="<skill_dir>/scripts/design-discovery/start-server.sh [--project-dir <project-root>]",
  background=true,
  watch_patterns=["server-started"]
)
```

The script runs in the foreground by design; Hermes owns the background process. Read initial output with `process(action="log", session_id="<id>")`. The `server-started` JSON provides `url`, `screen_dir`, and `state_dir`. Do not add shell-level `nohup`, `disown`, or `&` wrappers.
...
2. Write a new semantic HTML fragment such as `layout.html` or `layout-v2.html` into `screen_dir` with `write_file`; never reuse a filename.
3. Open or inspect the returned URL with `browser_navigate` when useful, and tell the user what is shown and where to open it.
...
5. Read events with `read_file`, merge them with the user's reply, then revise the current screen or advance.
...
```text
terminal(command="<skill_dir>/scripts/design-discovery/stop-server.sh <session_dir>")
```
...
Runtime resources: [server](../scripts/design-discovery/server.cjs), [start script](../scripts/design-discovery/start-server.sh), [stop script](../scripts/design-discovery/stop-server.sh), [frame](../scripts/design-discovery/frame-template.html), and [click helper](../scripts/design-discovery/helper.js).
