...
## Use when
- One or more delegated results exist and the controller must accept, reject, serialize, or integrate their bounded outputs.
...
## Required inputs
- task context; slice manifests/dependency order; results and durable artifacts; current revision/scope/state; actual worktree/diff or remote handles; protected surfaces; integration seams; and downstream proof needs.
...
6. Recompute the actual integrated change/evidence view and list affected domain, review-tier, and verification needs for the owning controller's next decision.
