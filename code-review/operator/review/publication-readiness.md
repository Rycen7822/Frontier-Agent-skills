...
This operator contract owns validation of [the schema](../../schemas/publication-readiness.schema.json). Local review and publication are separate artifacts and authorities.

- Bind one validated local review by exact ref and one `review_result_digest`, then bind the publication decision to the same current source revision and the review's explicit paths/coverage. The digest binds transferred bytes; it is not review correctness or publication authority.
...
Duplicate check or approval IDs, stale review bytes, source revision or reviewed-path drift, missing authority, and contradictory ready states are typed validation failures. Render hosted summaries only from the validated pair of local-review and publication-readiness artifacts.
