...
This repository is the development source for the dual-host `frontier-engineering/11.0.0` bundle. One `frontier-engineering-plugin` contains ten skills for Codex and Hermes Agent. Installed copies are separate from source; local edits take effect after rebuilding and reinstalling.
...
| [code-review](code-review/SKILL.md) | Assess changes, scoped snapshots, review comments and evidence records | 2.0.0 |
...
The plugin identity is `frontier-engineering-plugin` version 11.0.0 with display name `Frontier Engineering`. Its release layout is:
